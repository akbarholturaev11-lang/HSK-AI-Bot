"""Authenticated Android chat API; media is transient and never logged."""
import asyncio
...
def create_android_assistant_router(*, session_factory, settings_obj):
    router = APIRouter(prefix="/api/v3/android/assistant", tags=["android-assistant"])
    ...
    async def status(request: Request):
        return await perform(request, lambda session, user, service: service.status(user, request.query_params.get("channel", "play")))
