...
.cmv2-exp{background:#fff;border:1px solid #e5eaf2;border-radius:18px;padding:14px;margin:0 0 16px;box-shadow:0 10px 28px rgba(25,36,59,.07);}
.cmv2-exp-title{font-size:18px;font-weight:900;color:#202938;line-height:1.25;margin-bottom:5px;}
.cmv2-exp-sub{font-size:13px;font-weight:700;color:#687386;line-height:1.4;margin-bottom:11px;}
.cmv2-steps{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;}
.cmv2-step{min-height:30px;border-radius:10px;background:#f1f5fb;color:#6b7687;display:grid;place-items:center;text-align:center;font-size:11px;font-weight:850;padding:5px;}
.cmv2-step.active{background:#e8f4ff;color:#2473da;}
.cmv2-step.done{background:#effcf6;color:#16855a;}
...
`;
  ...
  async function loadLesson(state) {
    ...
    const response = await fetch(
      `/api/miniapp/lesson?level=${encodeURIComponent(state.level)}&lesson=${encodeURIComponent(state.lessonId)}&lang=${encodeURIComponent(state.lang)}&mode=${encodeURIComponent(state.mode)}${blockParam}`,
  ...
  function experienceStageIndex(state) {
    if (!state.tasks.length) return 0;
    const ratio = state.index / Math.max(state.tasks.length, 1);
  ...
  function renderExperience(state) {
    const exp = state.lesson?.experience;
    if (!exp?.pilot) return "";
    const labels = Array.isArray(exp.skeleton) ? exp.skeleton : [];
    const active = experienceStageIndex(state);
    return `
      <div class="cmv2-exp">
        <div class="cmv2-exp-title">${esc(exp.today || state.lesson.title || "")}</div>
        <div class="cmv2-exp-sub">${esc(exp.promise || "")}</div>
        <div class="cmv2-steps">
          ${labels.map((label, index) => {
            const cls = index < active ? "done" : (index === active ? "active" : "");
            return `<div class="cmv2-step ${cls}">${esc(label)}</div>`;
          }).join("")}
        </div>
      </div>
    `;
  ...
  async function renderResult(state) {
    ...
    const exp = state.lesson?.experience;
    const resultText = exp?.pilot ? [exp.win_text, exp.next_text].filter(Boolean).join(" ") : "";
    main.innerHTML = `
