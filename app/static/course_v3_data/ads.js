/* CourseAds — reklama moduli.
 *
 * Reklama AYNAN IKKI JOYDA chiqadi:
 *   • dars yakunida (`playLessonEnd`),
 *   • ekran markazida, qaysi bo'limda bo'lishidan qat'i nazar (`playScreenCenter`).
 *
 * Mashq sessiyalaridagi boshi/o'rtasi/oxiri uchligi OLIB TASHLANDI, reklama
 * ko'rib kirish ochish ham (u bilan birga urinish tokeni va binding tarmog'i).
 *
 * Kunlik chegara SERVERDA, Telegram akkaunti bo'yicha sanaladi — qurilma
 * almashtirish yoki brauzer xotirasini tozalash uni aylanib o'tmaydi. Bu
 * yerdagi `localStorage` faqat bitta ochilishda ikki marta chiqib qolmasligi
 * uchun; u chegara EMAS.
 *
 * Foydalanish:
 *   CourseAds.config({lang:"uz", initData:tg.initData, onSubscribe:function(){...}});
 *   CourseAds.playScreenCenter();
 *   CourseAds.playLessonEnd(lessonOrder);
 *
 * `showLimitPromo` — reklama emas: bu limitga urilganda chiqadigan paywall.
 * U shu moduldagi overlayni ishlatgani uchun shu yerda qoladi. */
(function(){
  ...
  var STATE = {timer:null,loadTimer:null,resolve:null,reject:null,ad:null,placement:"",watched:0,ready:false,busy:false};
  ...
  /* Serverdan shu JOY uchun reklama. Bo'sh javob XATO EMAS: chegara tugagan,
     joy o'chirilgan, foydalanuvchi obunachi yoki katalog bo'sh bo'lishi
  ...
  function fetchPlacementAd(placement){
    ...
    return fetch(url,{headers:{"X-Telegram-Init-Data":CFG.initData||""}})
      ...
      .then(function(d){
        var ad=d&&d.ad;
        if(!d||d.ok!==true||!ad||!ad.media_url||ad.media_available===false)return null;
        return ad;
      })
      .catch(function(){return null});
  }
  /* Ko'rilganini yozadi. Ilgari bu yerda `access_ref` va `attempt_token` ham
     bor edi — reklama ko'rib DARSNI OCHISH uchun. U mexanizm olib tashlandi,
     shuning uchun bu chaqiruv endi faqat hisob uchun. */
  function recordView(ad,placement,watched){
    if(!CFG.initData||!ad)return Promise.resolve({ok:false});
    return fetch("/api/v3/ad/view",{method:"POST",headers:{"Content-Type":"application/json","X-Telegram-Init-Data":CFG.initData},
      body:JSON.stringify({ad_id:ad.id,level:CFG.level||"hsk1",lesson_order:Number(CFG.lessonOrder)||0,placement:placement,watched_seconds:watched})})
  ...
  /* Mashq sessiyalaridagi reklama (boshi/o'rtasi/oxiri) OLIB TASHLANDI.
     Reklama endi aynan ikki joyda: dars yakuni va ekran markazi.

     Funksiya ATAYLAB qoldirilgan va darhol hal bo'ladi: chaqiruv joylari
     `CourseAds.play("start").then(start).catch(start)` shaklida yozilgan,
     ya'ni bu holda sessiya oddiygina reklamasiz boshlanadi. Chaqiruvlar
    ...
    if(!why){
      ...
      function fetchLimitStatus(){
        return fetch("/api/v3/limits/status",{method:"POST",headers:{"Content-Type":"application/json","X-Telegram-Init-Data":CFG.initData||""},body:JSON.stringify({feature:statusFeature})})
          .then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error("limit_status");return d})});
      }
      var statusPromise=null;
      try{if(window.CourseLimitStatus&&typeof window.CourseLimitStatus.get==="function")statusPromise=window.CourseLimitStatus.get(statusFeature)}catch(e){}
      (statusPromise||fetchLimitStatus()).catch(function(){return fetchLimitStatus()}).then(function(d){
  ...
  /* ============================================================
     ...
     Kunlik chegara SERVERDA. Quyidagi `localStorage` hisoblagichi faqat
     bitta ochilish ichidagi takrorni to'xtatadi — u chegara EMAS.
     ============================================================ */
  var appEls=null, appState={open:false,timer:null,shownInSession:false,ad:null,placement:"",watched:0};
  ...
  function closeCenterAd(){
    if(!appEls) return;
    /* Ko'rilgani serverga yoziladi: kunlik chegara aynan shu qatorlardan
       sanaladi, klientning localStorage hisoblagichidan emas. */
    try{
      if(appState.ad){
        recordView(appState.ad,appState.placement||"screen_center",appState.watched||0);
        appState.ad=null;
      }
    }catch(err){}
  ...
  /* Ekran markazidagi reklama — qaysi bo'limda bo'lishidan qat'i nazar.
     Kunlik chegara (kuniga 2) SERVERDA, Telegram akkaunti bo'yicha sanaladi:
     qurilma almashtirish yoki brauzer xotirasini tozalash uni aylanib
     o'tmaydi. Bu yerdagi `shownInSession` faqat bitta ochilishda ikki marta
     chiqib qolmasligi uchun — chegara emas. */
  function playScreenCenter(){
  ...
  function playLessonEnd(lessonOrder){
    ...
    /* Dars yakunida obuna kartasi va tashqi CTA chiqadi — `isLessonEnd()`
       shu bayroqqa qaraydi. */
    CFG.slot="lesson_end";
  ...
  window.CourseAds = {
    ...
    /* Reklama aynan ikki joyda. */
    playScreenCenter:playScreenCenter,
    playLessonEnd:playLessonEnd,
    closeCenterAd:closeCenterAd,
    /* Limit paywalli — reklama emas, lekin shu moduldagi overlayni ishlatadi. */
    showLimitPromo:showLimitPromo,
    closeLimit:_closeLimit,
    available:function(){return !!els},
    /* Eskirgan: mashq sessiyalaridagi reklama olib tashlandi. Chaqiruvlar
       tozalanguncha darhol hal bo'ladi va sessiya reklamasiz boshlanadi. */
    play:play,
    playAppOpen:playScreenCenter
  };
})();
