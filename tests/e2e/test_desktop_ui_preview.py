                ...
                if (command === "desktop_auth_status") {{
                  return {{
                    ...
                    bootstrap: {{
                      ok: true,
                      authenticated: true,
                      user: {{ name: "Fixture User", language: "uz", level: "hsk1", is_paid: true }}
                    }}
                  }};
                }}
...
def test_desktop_course_lesson_profile_and_ai_flow(
    ...
    assert "HSK 1" in page.locator("#content-subtitle").inner_text()
    ...
    page.get_by_role("heading", name="Profil va sozlamalar").wait_for()
    page.get_by_role("button", name="Русский", exact=True).click()
    page.get_by_role("heading", name="Профиль и настройки").wait_for()
...
def test_explicit_telegram_link_flow(desktop_ui_url, browser_page):
    ...
    assert page.locator("#auth-code").inner_text() == "HSK4821X"
...
def test_preview_half_stops_without_completion(
    ...
    assert "Telegram Mini App" in page.locator("#lesson-content").inner_text()
    assert page.evaluate("window.__COMPLETIONS.length") == 0
    assert errors == []
...
def test_stale_lesson_response_cannot_replace_newer_lesson(
    ...
    lesson_rows = page.locator(".lesson-row")
    lesson_rows.nth(0).click()
    page.locator("#close-lesson").click()
    lesson_rows.nth(1).click()
